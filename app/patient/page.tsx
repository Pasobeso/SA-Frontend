...
export default function AppointmentsPage() {
	...
	function toDateWithTimezone(dateString: string) {
		const utc = new Date(dateString + "Z"); // parse as UTC
		return utc;
	}
										...
										<div className="flex items-start justify-between">
											<div>
												...
												<h3 className="text-lg font-semibold text-gray-900 mb-1">
													วันที่{" "}
													{(toDateWithTimezone(appointment.start_time).toLocaleDateString(
														"th-TH",
														{ day: "numeric", month: "long", year: "numeric" }
													))}
												</h3>
												<p className="text-xl font-bold text-gray-900 mb-2">
													{toDateWithTimezone(appointment.start_time).toLocaleTimeString(
														"th-TH",
														{ hour: "2-digit", minute: "2-digit" }
													)} - {toDateWithTimezone(appointment.end_time).toLocaleTimeString(
														"th-TH",
														{ hour: "2-digit", minute: "2-digit" }
													)}
												</p>
