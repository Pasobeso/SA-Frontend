...
export namespace Booking {
  ...
  export async function deleteAppointment(appointmentId: string) {
    const res = await client.delete(
      `${BOOKINGS_URL}/appointment-ops/${appointmentId}`,
    );
  ...
  export async function markAsReady(appointmentId: string) {
    const res = await client.patch(
      `${BOOKINGS_URL}/appointment-ledger/to-ready/${appointmentId}`,
    );
    return res.data as ApiResponse<null>;
  }
  ...
  export async function markAsCompleted(appointmentId: string) {
    const res = await client.patch(
      `${BOOKINGS_URL}/appointment-ledger/to-completed/${appointmentId}`,
    );
    return res.data as ApiResponse<null>;
  }
